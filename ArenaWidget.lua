...
local alphabet_lower = {"a", "b", "c", "d", "e"}
local ArenaUnitIdToNumber = {}

-- replace enemies with numbers and friendlies with letters
for i = 1, MAX_ARENA_ENEMIES do
  ArenaUnitIdToNumber["arena" .. i] = i
  ArenaUnitIdToNumber["arenapet" .. i] = i
  --friendly
  ArenaUnitIdToNumber["party" .. i] = alphabet[i]
  ArenaUnitIdToNumber["partypet" .. i] = alphabet_lower[i]
end
...
-- enemy: update names whenever seen
...
-- friendly: update names whenever someone joins or group changes
...
-- clear namespace then register all events
...
-- enable changes only for allowed nameplates
function Widget:EnabledForStyle(style, unit)
  return not (style == "NameOnly" or style == "NameOnly-Unique" or style == "etotem")
end

-- control what happens to new nameplates
